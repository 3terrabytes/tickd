...
export default function FriendsPage() {
  ...
  useEffect(() => {
    if (tab==='find') {
      if (search.trim()) { loadAll(search); } else { setAllUsers([]); }
    }
  }, [tab, search, loadAll]);
      ...
      {tab==='find' && (
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8, marginTop:12 }}>
            ...
            {!loading && allUsers.length===0 && !search && <p style={{ color:'var(--text-muted)', fontSize:14, textAlign:'center', padding:24 }}>Type a username to search for players</p>}
          </div>
        </div>
      )}
