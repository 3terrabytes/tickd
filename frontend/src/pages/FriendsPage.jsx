...
function NameWithBanner({ user }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
      {banner && (
        <div style={{
          background: banner.color, borderRadius: 4, padding: '1px 7px',
...
function PlayerCard({ player, rank }) {
  ...
  return (
    <div style={{
      ...
      background: banner ? 'transparent' : 'var(--bg2)',
      backgroundImage: banner ? banner.color : undefined,
      border: `1px solid ${player.isSelf ? 'var(--accent)' : 'var(--border)'}`,
      borderRadius: 12, position: 'relative', overflow: 'hidden'
    }}>
      {/* Banner overlay */}
      {banner && <div style={{ position: 'absolute', inset: 0, background: banner.color, opacity: 0.15 }} />}
