...
export default function FriendsPage() {
  ...
  // Duel — async PvP. Server simulates a fight, returns the result, we display
          ...
          {friends.map(f => (
            <div key={f.id} className="card" style={{ ...S.friendRow, cursor: 'pointer' }}
              ...
              <div style={{ display: 'flex', gap: 6 }} onClick={e => e.stopPropagation()}>
                <button className="btn btn-ghost" style={{ padding: '6px 10px', fontSize: 11 }} title="Send a gift" onClick={() => openGift(f)}>🎁</button>
                <button className="btn btn-ghost" style={{ padding: '6px 10px', fontSize: 11 }} title="Propose a trade" onClick={() => openTrade(f)}>🔄</button>
                <button className="btn btn-ghost" style={{ padding: '6px 10px', fontSize: 11, color: '#fca5a5' }} title="Duel for XP" onClick={() => openDuel(f)}>🗡️</button>
...
// Duel result modal � shows the simulated PvP outcome with XP gained.
