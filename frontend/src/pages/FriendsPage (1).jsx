...
function Avatar({ user }) {
  if (user.equipped && (user.equipped.weapon || user.equipped.armor || user.equipped.banner || user.equipped.badge)) {
    return (
      <div style={{ position: 'relative', width: 40, height: 40, flexShrink: 0 }}>
        <PixelCharacter equipped={user.equipped} size={40} />
      </div>
    );
  }
  return (
...
function bannerStyle(banner) {
  ...
  const gradients = {
    common:    'linear-gradient(90deg, rgba(120,130,140,0.18) 0%, rgba(120,130,140,0.04) 100%)',
    rare:      'linear-gradient(90deg, rgba(74,111,165,0.22) 0%, rgba(74,111,165,0.05) 100%)',
    epic:      'linear-gradient(90deg, rgba(139,87,200,0.22) 0%, rgba(139,87,200,0.05) 100%)',
    legendary: 'linear-gradient(90deg, rgba(230,160,20,0.28) 0%, rgba(230,160,20,0.05) 100%)',
  };
  const borders = { common: '#78828c', rare: '#4a6fa5', epic: '#8b57c8', legendary: '#e6a014' };
  return { gradient: gradients[banner.rarity] || gradients.common, borderColor: borders[banner.rarity] || borders.common };
}
...
function PlayerCard({ player, rank, onClick }) {
  ...
  const bs        = bannerStyle(banner);
  const title     = player.equipped?.title;
  const companion = player.equipped?.companion;
    ...
    >
      {bs && (
        <div style={{
          ...
          borderLeft: `3px solid ${bs.borderColor}`,
          zIndex: 0,
        }} />
      )}

      <div style={{ fontSize: 20, width: 28, textAlign: 'center', flexShrink: 0, position: 'relative', zIndex: 1 }}>
        {rank <= 3 ? medals[rank - 1] : <span style={{ color: 'var(--text-muted)', fontSize: 14 }}>#{rank}</span>}
      </div>

      <div style={{ position: 'relative', zIndex: 1, flexShrink: 0 }}>
        <PixelCharacter equipped={player.equipped || {}} size={48} />
        {companion && (
          <div style={{
            position: 'absolute', bottom: -4, right: -6, fontSize: 16, lineHeight: 1,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {title && (
            <span style={{
              ...
              background: 'rgba(255,200,50,0.12)', color: '#f0b840',
              border: '1px solid rgba(255,200,50,0.25)', fontStyle: 'italic',
            }}>{title.name} {title.emoji}</span>
          )}
        </div>
      ...
      {!player.isSelf && <div style={{ fontSize: 12, color: 'var(--text-muted)', position: 'relative', zIndex: 1 }}>→</div>}
    </div>
  );
}
