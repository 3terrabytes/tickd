...
export default function Dashboard() {
  ...
  const complete = async (habit) => {
    ...
    try {
      ...
      const id = Date.now();
      setFloats(f => [...f, { id, xp: result.xpEarned, gold: result.goldEarned }]);
  ...
  const addHabit = async (e) => {
    ...
    setNewHabit({ name: '', icon: '⚡', color: '#6366f1', gold_reward: 10 });
    setShowAdd(false);
    await load();
  };
  ...
  return (
    <div style={styles.wrap}>
      ...
      {floats.map(f => (
        <div key={f.id} style={styles.floatWrap}>
          <div style={styles.xpFloat}>+{f.xp} XP</div>
          <div style={styles.goldFloat}>+{f.gold} 💰</div>
        </div>
      ))}
        ...
        <div style={styles.playerRow}>
          ...
          <div style={styles.statsCol}>
            <div style={styles.statItem}>
              ...
              <span style={{ fontWeight: 700, color: '#f59e0b' }}>{user?.gold || 0}</span>
            </div>
            <div style={styles.statItem}>
              <span style={{ fontSize: 18 }}>🔥</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {habits.map(habit => (
          <HabitCard key={habit.id} habit={habit} onComplete={() => complete(habit)} onDelete={() => deleteHabit(habit.id)} />
        ))}
      ...
      {showAdd ? (
        <form onSubmit={addHabit} style={{ ...styles.addCard, marginTop: 12 }} className="card animate-fade">
          ...
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Difficulty</div>
            <div style={{ display: 'flex', gap: 8 }}>
              {GOLD_OPTIONS.map(opt => (
                <button key={opt.value} type="button"
                  onClick={() => setNewHabit(f => ({ ...f, gold_reward: opt.value }))}
                  style={{
                    flex: 1, padding: '8px', borderRadius: 8, border: '1px solid',
                    borderColor: newHabit.gold_reward === opt.value ? 'var(--accent)' : 'var(--border)',
                    background: newHabit.gold_reward === opt.value ? 'rgba(99,102,241,0.15)' : 'var(--bg3)',
                    color: newHabit.gold_reward === opt.value ? 'var(--text)' : 'var(--text-muted)',
                    fontSize: 12, cursor: 'pointer'
                  }}
                >
                  {opt.label}
                  <div style={{ color: '#f59e0b', fontSize: 11, marginTop: 2 }}>💰 {opt.value} gold</div>
                </button>
              ))}
            </div>
          </div>
...
function HabitCard({ habit, onComplete, onDelete }) {
  ...
  const difficulty = habit.gold_reward >= 50 ? { label: 'Hard', color: '#ef4444' }
    : habit.gold_reward >= 30 ? { label: 'Medium', color: '#f59e0b' }
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {habit.streak > 0 && <span className="streak-fire">🔥{habit.streak}</span>}
          <span style={{ fontSize: 11, color: difficulty.color, fontWeight: 600 }}>{difficulty.label}</span>
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
          {habit.total_completions} total · Best: {habit.best_streak} days · 💰 {habit.gold_reward} gold
        </div>
      </div>

      {hovered && (
        <button onClick={onDelete} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 16, padding: '4px 8px', borderRadius: 6 }}>✕</button>
      )}
    </div>
  );
}
...
const styles = {
  ...
  floatWrap: {
    position: 'fixed', top: '40%', right: '20%', pointerEvents: 'none', zIndex: 200,
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
    animation: 'xpFloat 1.4s ease forwards'
  },
  xpFloat: { color: 'var(--accent2)', fontWeight: 700, fontSize: 16 },
  goldFloat: { color: '#f59e0b', fontWeight: 700, fontSize: 16 },
  ...
  levelBadge: { background: 'var(--accent)', color: 'white', padding: '2px 8px', borderRadius: 99, fontSize: 12, fontWeight: 600 },
  ...
  progressFill: { height: '100%', borderRadius: 99, background: 'linear-gradient(90deg, var(--green), #34d399)', transition: 'width 0.5s ease' },
  habitCard: { display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 12, border: '1px solid', transition: 'all 0.2s ease', cursor: 'default' },
  checkBtn: { width: 44, height: 44, borderRadius: '50%', border: '2px solid', fontSize: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s ease', color: 'white' },
  addCard: { background: 'var(--bg2)' },
  input: { width: '100%', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 14px', color: 'var(--text)', fontSize: 14, outline: 'none' },
  iconBtn: { width: 36, height: 36, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg3)', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
