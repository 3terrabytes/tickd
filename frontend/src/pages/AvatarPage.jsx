...
const RARITY = {
  junk:      { color: '#6b5c3e', label: 'Junk',      border: '#6b5c3e44' },
...
const SKIN_TONES   = ['#FDDBB4','#f5c8a0','#e8b88a','#d4956a','#c07840','#a05c28','#7a3c10','#4a1e06'];
const HAIR_COLORS  = ['#1a1a1a','#3d2b1a','#6b3a2a','#8B4513','#b5651d','#DAA520','#f0c040','#e8e8e8','#ffffff','#FF6B35','#e83a6a','#cc3399','#7733cc','#3366cc','#33aacc','#33cc88'];
const EYE_COLORS   = ['#1a1a2a','#2a4a8a','#2a7a3a','#8a3a2a','#7a6a2a','#5a2a7a','#1a6a7a','#3a8a6a','#8a5a2a','#cc6688'];
const HAIR_STYLE_NAMES = ['Spiky','Long','Short Crop','Ponytail'];
const BEARD_OPTIONS = [
  { label: 'None',        value: 0 },
  { label: 'Stubble',     value: 1 },
  { label: 'Short Beard', value: 2 },
  { label: 'Full Beard',  value: 3 },
  { label: 'Moustache',   value: 4 },
];
...
export default function AvatarPage() {
  ...
  const [appearance, setAppearance] = useState({
    ...
    avatar_eyes: '#2a4a8a', avatar_hair_style: 0, avatar_beard: 0
  });
  ...
  useEffect(() => {
    if (user) {
      setAppearance({
        ...
        avatar_beard:      user.avatar_beard      || 0,
      });
    }
  }, [user?.id]);
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                {BEARD_OPTIONS.map(opt => (
                  <button key={opt.value} type="button"
                    onClick={() => setAppearance(a => ({ ...a, avatar_beard:opt.value }))}
                    style={{
                      padding:'6px 12px', borderRadius:8, border:'1px solid',
                      borderColor: appearance.avatar_beard===opt.value ? 'var(--accent)' : 'var(--border)',
                      background: appearance.avatar_beard===opt.value ? 'rgba(99,102,241,0.15)' : 'var(--bg3)',
                      color: appearance.avatar_beard===opt.value ? 'var(--text)' : 'var(--text-muted)',
                      fontSize:12, cursor:'pointer'
                    }}>{opt.label}</button>
                ))}
              </div>
