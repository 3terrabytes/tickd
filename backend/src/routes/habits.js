...
router.post('/:id/complete', async (req, res) => {
  ...
  try {
    ...
    // Gold earned (streak bonus)
    const goldEarned = newStreak >= 14 ? 10 : newStreak >= 7 ? 8 : newStreak >= 3 ? 6 : 5;

    // Update user XP, gold and level
    const { rows: users } = await pool.query(
      'UPDATE users SET xp = xp + $1, gold = gold + $2 WHERE id = $3 RETURNING xp, level, gold',
